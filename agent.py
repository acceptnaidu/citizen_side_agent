...
from shared_libraries.utils import INTENT_INDEX, read_yaml_file
from dotenv import load_dotenv

load_dotenv()

YAML_READER = FunctionTool(func=read_yaml_file)
...
class RoutingAgent:
    ...
    def create_agent(self) -> Agent:
        return Agent(
            ...
            description=(
                """You are a Citizen Support Agent. Your primary function is to listen to citizen queries, collect all necessary requirements with help of {INTENT_INDEX} & `YAML_READER`, and route them to the appropriate available agents for resolution.
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return f"""
        **Role:** You are a Citizen Support Agent. Your primary function is to listen to citizen queries, collect all necessary requirements, and route them to the appropriate available agents for resolution.

        **Core Directives:**

        * **Listen and Collect:** Carefully listen to the citizen's query.
        * **Find workflow:** Based on the query, identify the appropriate workflow to follow from the {INTENT_INDEX} file.
        * **Read workflow:** Read the workflow from `YAML_READER` tool by using the intent file name and understand the steps involved and gather all necessary information.
        * **Task Delegation:** Utilize the `send_message` function to assign actionable tasks to remote agents based on the collected requirements.
